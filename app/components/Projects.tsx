    ...
    <section className="py-15 container max-w-7xl mx-auto px-4">
      ...
      >
        {projects.map((project: ProjectType) => {
          const hasDetails = !!(
            project.keyFeatures?.length || project.architectureHighlights
          );
          ...
          return (
            <motion.article
              ...
              variants={item}
              whileHover={{ y: -5 }}
              className="relative group bg-white dark:bg-gray-800/50 rounded-xl shadow-lg hover:shadow-xl dark:hover:shadow-gray-700/30 transition-all duration-300 overflow-hidden border border-gray-100 dark:border-gray-700"
            >
              {project.featured && (
                <span className="absolute top-3 left-3 z-10 flex items-center gap-1 text-xs font-medium text-cyan-400 bg-gray-900/80 backdrop-blur-sm px-2 py-1 rounded-full border border-cyan-500/30">
                ...
                {hasDetails && (
                  <div className="mb-4">
                    <button
                      ...
                      className="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-primary dark:hover:text-cyan-400 transition-colors"
                    >
                      <motion.span
